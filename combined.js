...
/* === AI Transcript: populate-only add-on (no mechanics changed) === */
;(function(){
  ...
  function chip(label){
    ...
    s.textContent=label;
    s.style.display='inline-flex'; s.style.alignItems='center'; s.style.gap='6px';
    s.style.background='#eaf2ff'; s.style.color='#1a73e8';
    s.style.borderRadius='12px'; s.style.padding='4px 8px';
    s.style.fontSize='12px'; s.style.fontWeight='700';
    return s;
  }
  function fmt(n){ n=Math.max(0,Math.floor(n)); var m=Math.floor(n/60),s=n%60; return m+':' + (s<10?'0'+s:s); }
  function secsFromDur(d){ var p=String(d||'0:00').split(':'), s=0; for(var i=0;i<p.length;i++){ s=s*60+(+p[i]||0);} return s; }
  ...
  document.addEventListener('click', function(ev){
    var btn = ev.target && ev.target.closest ? ev.target.closest('button[data-action="transcript"]') : null;
    if (!btn) return;

    // Let your existing handler open/build the modal first
    setTimeout(function(){
      ...
      if (!chips || !summary || !segWrap || !range || !clock) return;
      ...
      // Chips
      chips.innerHTML='';
      chips.appendChild(chip('From: ' + (from||'—')));
      chips.appendChild(chip('To: '   + (to || dial || '—')));
      chips.appendChild(chip('⏱ '     + (dur||'0:00')));
      chips.appendChild(chip('📅 '     + (date||'—')));
      ...
      range.max = String(total); range.value='0'; clock.textContent='0:00';
      ...
      for (var j=0;j<segs.length;j++){
        ...
        item.className='cv-ai-seg'; item.setAttribute('data-t', String(s.t0));
        item.style.border='1px solid #e5e7eb'; item.style.borderRadius='10px';
        item.style.padding='12px'; item.style.margin='10px 0'; item.style.cursor='pointer';

        var rowT = document.createElement('div');
        rowT.style.display='flex'; rowT.style.alignItems='center'; rowT.style.gap='8px';
        ...
        dot.style.background='#2563eb'; dot.style.display='inline-block';
        ...
        var body=document.createElement('div'); body.textContent=s.text; body.style.marginTop='8px';
      ...
      // Jump to segment
      segWrap.onclick = function(e2){
        var n = e2.target;
        while (n && n!==segWrap){
          if (n.className==='cv-ai-seg'){
            var t = Number(n.getAttribute('data-t'))||0;
            range.value=String(t); clock.textContent=fmt(t);
            break;
          }
          n = n.parentNode;
        }
      ...
      // Play/Pause (rebind fresh each open)
      var timer=null;
      play.onclick = function(){
        ...
        timer=setInterval(function(){
          ...
          range.value=String(v); clock.textContent=fmt(v);
        }, 1000);
      };
