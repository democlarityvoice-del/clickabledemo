  ...
  function ensureHost(){
    if (!document.getElementById(STYLE_ID)){
      ...
      s.textContent = [
        ...
        '#',ROOT_ID,' thead th:nth-child(5), #',ROOT_ID,' tbody td:nth-child(5){width:86px}',
        ...
        '#',ROOT_ID,' input[type="number"], #',ROOT_ID,' select{max-width:72px}',
...
(function CVReportsOverlay() {
  ...
  function buildSrcdoc() {
    ...
    // Build row HTML for initial table rendering (client-side template build)
    function rowHTMLBuild(r){
      const click = (v)=>`<span class=\"click\" role=\"button\" tabindex=\"0\">${v}</span>`;
      const maybeAbandon = (v)=> String(v).endsWith('%') && parseFloat(v)===0 ? `<span class=\"gray\">${v}</span>` : click(v);
      return `
        <tr>
          <td>${r.q}</td>
          <td>${r.name}</td>
          <td class=\"num\">${click(r.vol)}</td>
          <td class=\"num\">${click(r.handled)}</td>
          <td class=\"num\">${click(r.offered)}</td>
          <td class=\"num\">${click(r.talk)}</td>
          <td class=\"num\">${maybeAbandon(r.abandon)}</td>
          <td class=\"num\">${click(r.handle)}</td>
          <td class=\"num\">${click(r.wait)}</td>
        </tr>`;
    }
...
<body>
    ...
    <div class="card">
      ...
      <div style="overflow:auto; max-height: 520px;">
        <table aria-label="Queue stats table" id="table">
          ...
          <tbody>
            ${rows.map(r => rowHTMLBuild(r)).join('')}
          </tbody>
        </table>
      </div>
    </div>
  ...
  <script>
    ...
    setInterval(()=>{ const dt=new Date(); const mm=String(dt.getMonth()+1).padStart(2,'0'); const dd=String(dt.getDate()).padStart(2,'0'); const yyyy=dt.getFullYear(); let h=dt.getHours(); const m=String(dt.getMinutes()).padStart(2,'0'); const am=h>=12?'pm':'am'; h=h%12; if(h===0)h=12; document.getElementById('upTo').textContent = 'Showing data up to ' + `${mm}/${dd}/${yyyy} ${h}:${m} ${am}`; }, 60000);
...
/* =========================
   CV Reports Overlay v1.1 — APPEND-ONLY, ISOLATED
   Safe to paste AFTER line 2397. Does not touch existing code.
   ========================= */
(function(){
  'use strict';
  // ---- HARD GUARDS (path + idempotency) ----
  if (location.pathname.indexOf('/portal/stats/queuestats/queue') === -1) return;
  if (document.getElementById('cv-rpt-v1-host')) return; // already injected

  function init(){
    try{
      var host = document.createElement('div');
      host.id = 'cv-rpt-v1-host';
      host.style.cssText = 'position:relative;z-index:2147483000;margin:8px 0 16px;background:#fff;border:1px solid #dcdcdc;border-radius:8px;box-shadow:0 1px 2px rgba(0,0,0,.06)';

      var iframe = document.createElement('iframe');
      iframe.id = 'cv-rpt-v1-iframe';
      iframe.title = 'Clarity Voice – Reports (Queue Stats) Overlay v1.1';
      iframe.setAttribute('frameborder','0');
      iframe.style.cssText = 'width:100%;height:960px;border:0;display:block;';
      iframe.srcdoc = (function(){
        var brand = '#e57027';
        var blue = '#0B76C5';
        var now = new Date();
        var msHour = 3600*1000;
        var d = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        var todayMidnight = d.getTime();
        var points = [
          { t: todayMidnight - 24*msHour, v: 0,  lbl: 'Yesterday, 12:00 am' },
          { t: todayMidnight - 19*msHour, v: 0,  lbl: 'Yesterday, 5:00 am' },
          { t: todayMidnight - 14*msHour, v: 10, lbl: 'Yesterday, 10:00 am' },
        ...
        function fmtUpTo(ts){
          var dt = new Date(ts); var h = dt.getHours(); var m = String(dt.getMinutes()).padStart(2,'0');
          var am = h>=12?'pm':'am'; h = h%12; if(h===0) h=12;
          return String(dt.getMonth()+1).padStart(2,'0') + '/' + String(dt.getDate()).padStart(2,'0') + '/' + dt.getFullYear() + ' ' + h + ':' + m + ' ' + am;
        }
        function fmtDate(ts){ var dt=new Date(ts); return String(dt.getMonth()+1).padStart(2,'0') + '/' + String(dt.getDate()).padStart(2,'0') + '/' + dt.getFullYear(); }

        var csv = [['Queue','Name','Call Volume','Calls Handled','Calls Offered','Avg. Talk Time','Abandon Rate','Avg. Handle Time','Avg. Wait Time']]
          .concat(rows.map(function(r){return [r.q,r.name,r.vol,r.handled,r.offered,r.talk,r.abandon,r.handle,r.wait];}))
          .map(function(r){return r.map(function(v){return '"'+String(v).replace(/"/g,'""')+'"';}).join(',');}).join('\n');

        return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<style>
:root{ --brand:${brand}; --blue:${blue}; --muted:#6b6f73; --line:#e5e7eb; }
html,body{ margin:0; padding:0; font-family: Helvetica, Arial, sans-serif; color:#333; }
.wrap{ padding:16px 16px 24px; }
.tabs{ display:flex; gap:24px; font-size:14px; margin-bottom:8px; }
.tabs a{ color:#2367a2; text-decoration:none; }
...
.btn{ border:1px solid #d1d5db; background:#f8f9fa; padding:6px 10px; border-radius:6px; font-size:13px; cursor:pointer; }
.btn:hover{ background:#f3f4f6; }
.picker,.metric-select{ border:1px solid #d1d5db; border-radius:6px; padding:6px 8px; font-size:13px; background:#fff; }
.card{ border:1px solid #e5e7eb; border-radius:8px; padding:12px; background:#fff; }
.card + .card{ margin-top:12px; }
.chart-wrap{ width:100%; height:360px; position:relative; }
.chart{ width:100%; height:100%; display:block; }
...
<div class="wrap">
  ...
  <div class="toolbar">
    <div class="left">
      <input class="picker" type="text" value="${fmtDate(points[0].t)}" aria-label="Start date"/>
      <select class="picker"><option>12:00 am</option></select>
      <span>to</span>
      <input class="picker" type="text" value="${fmtDate(points[points.length-1].t)}" aria-label="End date"/>
      <select class="picker"><option>11:59 pm</option></select>
    </div>
    <div class="right">
      <button class="btn" aria-label="Settings">Settings</button>
      <button class="btn" aria-label="Email Reports">Email Reports</button>
    </div>
  </div>
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
    ...
    <div class="chart-wrap">
      <svg id="chart" class="chart" role="img" aria-label="Line chart" xmlns="http://www.w3.org/2000/svg"></svg>
      <div id="tooltip" class="tooltip" style="display:none"></div>
    </div>
  </div>
  <div class="card">
    <div class="meta" id="upTo">Showing data up to ${fmtUpTo(Date.now())}</div>
    <div style="overflow:auto; max-height:520px;">
      <table aria-label="Queue stats table" id="table">
        <thead>
          <tr>
            <th style="width:62px;">Queue</th>
            <th>Name</th>
            <th class="num" title="Total inbound + outbound events entering queues in the selected window.">Call Volume<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Calls answered by agents (excludes abandons/transfers).">Calls Handled<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Total calls presented to queues (includes handled + abandoned).">Calls Offered<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Average talk time for handled calls.">Avg. Talk Time<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Percentage of offered calls that abandoned before agent answer.">Abandon Rate<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Average handle time (talk + after-call work).">Avg. Handle Time<span class="header-help">ℹ︎</span></th>
            <th class="num" title="Average customer wait time before answer.">Avg. Wait Time<span class="header-help">ℹ︎</span></th>
          </tr>
        </thead>
        <tbody>
          ${rows.map(function(r){
            function clickable(v){ return '<span class="click" role="button" tabindex="0">'+v+'</span>'; }
            function maybeAbandon(v){ return (String(v).endsWith('%') && parseFloat(v)===0) ? '<span class="gray">'+v+'</span>' : clickable(v); }
            return '<tr>'+
              '<td>'+r.q+'</td>'+
              '<td>'+r.name+'</td>'+
              '<td class="num">'+clickable(r.vol)+'</td>'+
              '<td class="num">'+clickable(r.handled)+'</td>'+
              '<td class="num">'+clickable(r.offered)+'</td>'+
              '<td class="num">'+clickable(r.talk)+'</td>'+
              '<td class="num">'+maybeAbandon(r.abandon)+'</td>'+
              '<td class="num">'+clickable(r.handle)+'</td>'+
              '<td class="num">'+clickable(r.wait)+'</td>'+
            '</tr>'; }).join('')}
        </tbody>
      </table>
    </div>
...
<script>
  ...
  function render(){
    var w = svg.clientWidth || svg.parentElement.clientWidth; var h = svg.clientHeight || 360;
    svg.setAttribute('viewBox','0 0 '+w+' '+h); while(svg.firstChild) svg.removeChild(svg.firstChild);
    var padL=40,padR=10,padT=10,padB=24; var innerW=w-padL-padR, innerH=h-padT-padB;
    var max=Math.max(20, ...points.map(function(p){return p.v;}));
    function X(i){ return padL + (innerW * i/(points.length-1)); }
    function Y(v){ return padT + innerH - (v/max)*innerH; }
    var ln=document.createElementNS('http://www.w3.org/2000/svg','line');
    ln.setAttribute('x1',padL); ln.setAttribute('x2',w-padR); ln.setAttribute('y1',padT+innerH); ln.setAttribute('y2',padT+innerH); ln.setAttribute('class','axis'); svg.appendChild(ln);
    var path=document.createElementNS('http://www.w3.org/2000/svg','path');
    var d=points.map(function(p,i){return (i?'L':'M')+X(i)+' '+Y(p.v);}).join(' '); path.setAttribute('d',d); path.setAttribute('class','series'); svg.appendChild(path);
    points.forEach(function(p,i){ var c=document.createElementNS('http://www.w3.org/2000/svg','circle'); c.setAttribute('cx',X(i)); c.setAttribute('cy',Y(p.v)); c.setAttribute('r',3); c.setAttribute('class','pt');
      c.addEventListener('mouseenter', function(ev){ var rect=svg.getBoundingClientRect(); tip.style.display='block'; tip.textContent=p.lbl+' — All Queues: '+p.v; tip.style.left=(ev.clientX-rect.left)+'px'; tip.style.top=(ev.clientY-rect.top)+'px';});
      c.addEventListener('mouseleave', function(){ tip.style.display='none'; }); svg.appendChild(c); });
  }
  render(); var to; window.addEventListener('resize', function(){ clearTimeout(to); to=setTimeout(render, 100); });
  document.getElementById('metric').addEventListener('change', function(e){ document.getElementById('metricLabel').textContent=e.target.value; });
  document.getElementById('btn-print').onclick=function(){ try{window.print();}catch(e){} };
  document.getElementById('btn-download').onclick=function(){ var blob=new Blob([csv+'\n'],{type:'text/csv;charset=utf-8;'}); var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; a.download='queue-stats.csv'; document.body.appendChild(a); a.click(); a.remove(); setTimeout(function(){URL.revokeObjectURL(url);},500); };
  setInterval(function(){ document.getElementById('upTo').textContent='Showing data up to '+ '${fmtUpTo(Date.now())}'.replace(/\$/,''); }, 60000);
  document.getElementById('table').addEventListener('click', function(e){ var t=e.target; if(!t.classList.contains('click')) return; console.log('[cvRpt] detail click:', t.textContent); });
</script>
</body></html>`;
      })();
      ...
      content.appendChild(host); // append-only (no DOM deletions)

      // keep chart from getting tiny
      function resize(){ var h = Math.max(820, Math.round(window.innerHeight*0.9)); iframe.style.height = h + 'px'; }
      resize(); window.addEventListener('resize', function(){ try{ resize(); }catch(e){} });
    }catch(e){ console.error('[cvRpt] init error', e); }
  }
