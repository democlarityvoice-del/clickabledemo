...
;(function aiTranscriptModule(){
  ...
  function ensureAIModal(){
    ...
    document.addEventListener('keydown', function(e){ if (m.style.display==='block' && e.key==='Escape') close(); });
  ...
  function aiSummaryFor(type){
    return (type==='inbound')
      ...
      : 'Agent placed an outreach call and provided information and next steps. No commitments were made; follow-up can be scheduled.';
  }
  ...
  function openAI(row, type){
    ...
    var parts = String(dur).split(':'), secsTotal = 0;
    ...
    var items = segs.map(function(s){
      ...
      '<div class="cv-ai-seg" data-t="'+s.t0+'" style="border:1px solid #e5e7eb;border-radius:10px;padding:12px;margin:10px 0;cursor:pointer">'+
        '<div style="display:flex;align-items:center;gap:8px;color:#2563eb;font-weight:700">'+
          ...
          '<span>'+s.t0+'s</span><span style="color:#94a3b8;font-weight:600">– '+s.t1+'s</span>'+
        '</div>'+
    ...
    Array.prototype.forEach.call(m.querySelectorAll('.cv-ai-seg'), function(el){
      el.addEventListener('click', function(){ setPos(Number(el.getAttribute('data-t'))||0); });
    });
    ...
    m.querySelector('#cv-ai-btn-txt').onclick = function(){
      var lines = ['Summary:\n'+aiSummaryFor(type), '\n\nSegments:\n'];
      for (var j=0;j<segs.length;j++){ var s=segs[j]; lines.push('['+s.t0+'s–'+s.t1+'s] '+s.text+'\n'); }
  ...
  // click binding on Transcript icon (detect inbound vs outbound from "To" col)
  document.addEventListener('click', function(e){
    ...
    var tds = tr ? tr.querySelectorAll('td') : [];
    var row = {
      from:     (tds[1] && tds[1].textContent || '').trim(),
      dialed:   (tds[3] && tds[3].textContent || '').trim(),
      to:       (tds[5] && tds[5].textContent || '').trim(),
      date:     (tds[7] && tds[7].textContent || '').trim(),
      duration: (tds[8] && tds[8].textContent || '').trim()
    };
  ...
  console.log('[ai] bound');
})(); /* end aiTranscriptModule */
