...
(() => {
  ...
  const container = emptyCell?.closest('.table-container') || document.querySelector('.table-container');
  const wrapper   = container ? container.querySelector('.dataTables_wrapper') : null;
  if (!container) return;

  // If native table actually has rows, do nothing
  const hasRealRow = !!container.querySelector('tbody tr:not(.dataTables_empty)');
  if (hasRealRow) return;

  // Keep layout but hide the empty table visuals
  ...
  const ALLOWED = new Set(['(248) 436-3443','(313) 995-9080']);
  const DNIS_NAMES = {
    '(248) 436-3443': 'Clarity Main',
    '(313) 995-9080': 'Billing Hotline DID'
  };

  // Group by allowed DNIS
  const groups = {};
  CALLS.forEach(r => { if (ALLOWED.has(r.dnis)) (groups[r.dnis] ||= []).push(r); });

  // ----------------- Overlay DOM -----------------
  const overlay = document.createElement('div');
  overlay.id = 'cv-dnis-overlay';
  overlay.style.cssText = 'position:absolute;inset:0;z-index:5;pointer-events:auto;';
  overlay.innerHTML = `
  ...
  // ----------------- Summary rows -----------------
  const tbody = document.getElementById('cv-dnis-body');
  ['(248) 436-3443','(313) 995-9080'].forEach(dnis => {
    const rows = groups[dnis] || [];
    const vol = rows.length;
    const handled = vol; // demo: all handled
    ...
    tr.innerHTML = `
      ...
      <td style="padding:10px;color:#666;">${DNIS_NAMES[dnis]||''}</td>
      <td style="padding:10px;text-align:right;"><a href="#" class="cv-dnis-go" data-dnis="${dnis}">${vol}</a></td>
      <td style="padding:10px;text-align:right;"><a href="#" class="cv-dnis-go" data-dnis="${dnis}">${handled}</a></td>
      <td style="padding:10px;text-align:right;"><a href="#" class="cv-dnis-go" data-dnis="${dnis}">${fmt(avgTalk)}</a></td>
      <td style="padding:10px;text-align:right;"><a href="#" class="cv-dnis-go" data-dnis="${dnis}">${fmt(avgWait)}</a></td>
    `;
    tbody.appendChild(tr);
  });

  // ----------------- Details wiring -----------------
  const details   = document.getElementById('cv-dnis-details');
  const callsBody = document.getElementById('cv-dnis-calls');
  const titleEl   = document.getElementById('cv-dnis-title');

  function openDetails(dnis){
    titleEl.textContent = `${DNIS_NAMES[dnis]||''} — ${dnis}`;
    callsBody.innerHTML = '';
    (groups[dnis]||[]).forEach((r,i)=>{
      ...
      tr.innerHTML = `
        ...
        <td class="cvas-action-cell" style="padding:8px 10px;">
          <span data-action="listen"     style="cursor:pointer;margin-right:6px;">🎧</span>
          <span data-action="transcript" style="cursor:pointer;margin-right:6px;">📄</span>
          <span data-action="notes"      style="cursor:pointer;margin-right:6px;">📝</span>
          <span data-action="magnify"    style="cursor:pointer;margin-right:6px;">🔍</span>
          <span data-action="download"   style="cursor:pointer;">⬇️</span>
        </td>
      `;
  ...
  overlay.querySelectorAll('.cv-dnis-go').forEach(a =>
    a.addEventListener('click', e => { e.preventDefault(); openDetails(a.dataset.dnis); })
  );
  document.getElementById('cv-dnis-back')
    .addEventListener('click', ()=>{ details.style.display = 'none'; });

  document.getElementById('cv-dnis-search')
