...
;(function wireAITranscript(){
  try {
    ...
    function openAI(row, type){
      ...
      for (var i2=0;i2<segEls.length;i2++){
        (function(el){ el.addEventListener('click', function(){ setPos(Number(el.getAttribute('data-t'))||0); }); })(segEls[i2]);
      }
      ...
      // downloads (fake but functional; no template strings)
    ...
    // If anything here blows up, it won't take down the rows render.
    console.error('[AI Transcript init] failed:', aiErr);
  }
})();
