...
// === DIALED NUMBER STATS OVERLAY INJECTION ===(() => {
  // -------------------- Scope & one-time guard --------------------
  const ON_DNIS = /\/stats\/queuestats\/dnis(?:[?#].*)?$/i;
  if (!ON_DNIS.test(location.pathname)) return;
  if (window.cvDnisOverlayV5) return;
  window.cvDnisOverlayV5 = true;

  const log  = (...a)=>console.debug('%c[DNIS V5]', 'color:#6a5acd;font-weight:700', ...a);
  const warn = (...a)=>console.warn('%c[DNIS V5]', 'color:#6a5acd;font-weight:700', ...a);

  // -------------------- Helpers --------------------
  const secs = t => { const [m,s]=(t||'0:00').split(':').map(Number); return (m||0)*60+(s||0); };
  const fmt  = n => { const m=Math.floor(n/60), s=String(n%60).padStart(2,'0'); return `${m}:${s}`; };

  function findAnchors() {
    const emptyCell = document.querySelector('td.dataTables_empty, .dataTables_empty');
    const container = emptyCell?.closest('.table-container')
                   || document.querySelector('.table-container')
                   || document.querySelector('#modal_stats_table_wrapper')?.parentElement
                   || null;
    const wrapper   = container ? container.querySelector('.dataTables_wrapper') : null;
    const hasReal   = !!(container && container.querySelector('tbody tr:not(.dataTables_empty)'));
    return { emptyCell, container, wrapper, hasReal };
  }

  function waitForContainer(maxTries = 240) { // ~12s @ 50ms
    return new Promise(resolve => {
      (function tick(tries=0){
        const a = findAnchors();
        if (a.container) return resolve(a);
        if (tries >= maxTries) { resolve(a); return; }
        setTimeout(() => tick(tries+1), 50);
      })();
    });
  }

  function waitForTableState(expectEmpty=true, maxTries=240){
    return new Promise(resolve => {
      (function spin(tries=0){
        const a = findAnchors();
        if (!a.container) { 
          if (tries >= maxTries) return resolve(a);
          return setTimeout(()=>spin(tries+1),50);
        }
        const isEmpty = !!a.container.querySelector('.dataTables_empty') || !a.container.querySelector('tbody tr:not(.dataTables_empty)');
        if (expectEmpty ? isEmpty : !isEmpty) return resolve(a);
        if (tries >= maxTries) return resolve(a);
        setTimeout(()=>spin(tries+1),50);
      })();
    });
  }

  // -------------------- Data (your constants, 2 DNIS only) --------------------
  const CALLS = [
    // (248) 436-3443
  ...
  const ORDER = ['(248) 436-3443','(313) 995-9080'];
  const NAMES = { '(248) 436-3443':'Clarity Main', '(313) 995-9080':'Billing Hotline DID' };

  // -------------------- Render overlay --------------------
  function renderOverlay(container, wrapper) {
    if (document.getElementById('cv-dnis-overlay')) return;
    log('Rendering overlay…');

    // Keep layout but hide empty table visuals
    ...
    overlay.innerHTML = `
      ...
      <div id="cv-dnis-details" style="display:none;margin-top:10px;">
        <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#f9fafb;border:1px solid #e5e7eb;border-bottom:0;border-radius:6px 6px 0 0;">
          <button id="cv-dnis-back" style="border:1px solid #d8d8d8;border-radius:6px;padding:6px 10px;background:#fff;cursor:pointer;">← Back</button>
          <div id="cv-dnis-title" style="font-weight:700;">DNIS Details</div>
          <div style="flex:1"></div>
          <input id="cv-dnis-search" placeholder="Search calls…" style="border:1px solid #d8d8d8;border-radius:6px;padding:6px 8px;width:260px;">
        </div>
    ...
    // Summary rows
    const groups = CALLS.reduce((m,r)=>{ (m[r.dnis] ||= []).push(r); return m; }, {});
    const body = overlay.querySelector('#cv-dnis-body');

    ORDER.forEach(dnis => {
      const rows = groups[dnis] || [];
      const vol = rows.length;
      const handled = vol;
      const avgTalk = vol ? Math.round(rows.reduce((a,r)=>a+secs(r.talk),0)/vol) : 0;
      const avgWait = vol ? Math.round(rows.reduce((a,r)=>a+secs(r.wait),0)/vol) : 0;
    ...
    // Details wiring
    const details   = overlay.querySelector('#cv-dnis-details');
    const callsBody = overlay.querySelector('#cv-dnis-calls');
    const titleEl   = overlay.querySelector('#cv-dnis-title');

    function openDetails(dnis) {
      titleEl.textContent = `${NAMES[dnis]||''} — ${dnis}`;
      callsBody.innerHTML = '';
      (CALLS.filter(r=>r.dnis===dnis)).forEach((r,i) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          ...
          <td class="cvas-action-cell" style="padding:8px 10px;">🎧 📄 📝 🔍 ⬇️</td>
        `;
    ...
    overlay.querySelectorAll('.cv-go').forEach(a =>
      a.addEventListener('click', e => { e.preventDefault(); openDetails(a.dataset.dnis); })
    );
    overlay.querySelector('#cv-dnis-back')
      .addEventListener('click', ()=>{ details.style.display = 'none'; });

    overlay.querySelector('#cv-dnis-search')
      .addEventListener('input', e => {
        const t = e.target.value.toLowerCase();
        callsBody.querySelectorAll('tr').forEach(tr => {
          tr.style.display = tr.textContent.toLowerCase().includes(t) ? '' : 'none';
        });
      });

    // Auto-teardown if native data appears
    const mo = new MutationObserver(() => {
      if (container.querySelector('tbody tr:not(.dataTables_empty)')) {
        overlay.remove();
        if (wrapper) wrapper.style.visibility = '';
        mo.disconnect();
        log('Native table gained rows; overlay removed.');
      }
    });
  ...
  // -------------------- Boot: wait → check → render --------------------
  (async () => {
    const a1 = await waitForContainer();
    if (!a1.container) { warn('No container after waiting. Abort.'); return; }
    log('Container located.');

    const a2 = await waitForTableState(true);
    if (a2.hasReal) { log('Native table has rows; nothing to do.'); return; }

    renderOverlay(a2.container, a2.wrapper);
  })();
})();
