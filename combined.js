...
/* ===== CV Agents: Queues FAKE MODAL (append-only, no <script> wrapper) ===== */
(function(){
  if (window.__cvQueuesFakeModal_v2) return;
  window.__cvQueuesFakeModal_v2 = true;

  var PANEL_ID   = 'cv-agents-panel';
  var ROOT_ID    = 'cvqf-root';
  var STYLE_ID   = 'cvqf-style';
  var DATA_ATTR  = 'data-tool';
  var OLD_ATTR   = 'queues';
  var NEW_ATTR   = 'queues-fake';
  ...
  function stopAll(ev){ if (!ev) return; ev.preventDefault(); ev.stopPropagation(); if (ev.stopImmediatePropagation) ev.stopImmediatePropagation(); }

  function parseAgentFromRow(row){
    var label = ((row && row.querySelector('.cv-name'))||{}).textContent||'';
    var mExt = label.match(/Ext\.?\s*(\d{2,6})/i);
    var mNm  = label.match(/\(([^)]+)\)/);
    return { ext: (mExt && mExt[1]) || '200', name: (mNm && mNm[1]) || 'Agent' };
  }
  function guessDomainFromRow(row){
    try{
      var a = row.querySelector('a[href*="/portal/agents/agentrouting/"]');
      if (a && a.getAttribute('href')) {
        var dec = decodeURIComponent(a.getAttribute('href'));
        var m = dec.match(/sip:\d+@([a-z0-9_-]+)/i);
        if (m) return m[1];
      }
    }catch(_){}
    return 'claritydemo';
  ...
  // --- neutralize old queues handlers by retagging buttons ---
  function retagQueuesButtons(doc){
    doc = doc || document;
    var list = doc.querySelectorAll(
      '#'+PANEL_ID+' .cv-tools['+DATA_ATTR+'] ['+DATA_ATTR+'="'+OLD_ATTR+'"],'
      +'#'+PANEL_ID+' .cv-tools ['+DATA_ATTR+'="'+OLD_ATTR+'"]'
    );
    for (var i=0;i<list.length;i++){
      var el = list[i];
      el.setAttribute(DATA_ATTR, NEW_ATTR);
      if (el.tagName === 'A') { el.setAttribute('href', '#'); el.removeAttribute('onclick'); }
    }
  }
  retagQueuesButtons(document);
  new MutationObserver(function(){ retagQueuesButtons(document); })
    .observe(document.documentElement, { childList:true, subtree:true });
  ...
  function ensureHost(){
    if (!document.getElementById(STYLE_ID)){
      ...
      + '#'+ROOT_ID+' .cvqf-dialog{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);'
        +'background:#fff;border-radius:8px;box-shadow:0 12px 30px rgba(0,0,0,.18);'
        +'min-width:640px;max-width:980px;width:980px;max-height:80vh;display:flex;flex-direction:column;overflow:auto}'
      ...
      + '#'+ROOT_ID+' .cvqf-bd{padding:0 16px 12px;overflow:auto;background:#fff}'
      ...
      + '#'+ROOT_ID+' thead th{font:600 13px/1.2 "Helvetica Neue", Arial;color:#222;text-align:left;padding:10px 8px;white-space:nowrap;border-bottom:1px solid #eee}'
      + '#'+ROOT_ID+' tbody td{font:400 13px/1.35 "Helvetica Neue", Arial;color:#333;padding:10px 8px;border-bottom:1px solid #f0f0f0}'
      + '#'+ROOT_ID+' tbody tr:hover{background:#fafafa}'
      + '#'+ROOT_ID+' .num{text-align:center}'
      + '#'+ROOT_ID+' select{height:28px;font:600 13px/1 Arial}'
      + '#'+ROOT_ID+' .pill{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:4px;'
      ...
      + '#'+ROOT_ID+' .btn{padding:7px 12px;border-radius:6px;border:1px solid #d9d9d9;background:#fff;cursor:pointer;font:600 13px/1 Arial}'
    ...
    if (!document.getElementById(ROOT_ID)){
      ...
      root.addEventListener('click', function(ev){
        ...
        if (t.closest && t.closest('#cvqf-setstatus-menu a')) {
          stopAll(ev);
          var a = t.closest('a');
      ...
      document.addEventListener('click', function(ev){
        var inFooter = ev.target.closest && ev.target.closest('#cvqf-setstatus, #cvqf-setstatus-menu');
        if (!inFooter) $('#cvqf-setstatus-menu') && ($('#cvqf-setstatus-menu').style.display='none');
        var rowMenus = $all('#'+ROOT_ID+' .menu');
        for (var i=0;i<rowMenus.length;i++){
          if (!ev.target.closest || !ev.target.closest('#'+ROOT_ID+' .menu, #'+ROOT_ID+' button.pill')) rowMenus[i].style.display='none';
        }
      });
    }
  }
  ...
  function buildRows(agent){
    ...
    var offlineDefault = (agent.ext==='202' || agent.ext==='203'); // Jake & Bob
    var out = [];
  ...
  function wireRowMenus(root){
    ...
    tbody.addEventListener('click', function(ev){
      var pill = ev.target && ev.target.closest && ev.target.closest('button.pill');
      if (pill){
        stopAll(ev);
      ...
      var a = ev.target && ev.target.closest && ev.target.closest('.menu a');
      if (a){
        stopAll(ev);
        var makeOn = a.getAttribute('data-status')==='on';
        var wrap2 = a.parentNode.parentNode;
  ...
  // --- open modal on "queues" icon (now retagged to queues-fake) ---
  document.addEventListener('click', function(ev){
    var btn = ev.target && ev.target.closest && ev.target.closest('#'+PANEL_ID+' .cv-tools ['+DATA_ATTR+'="'+NEW_ATTR+'"]');
    if (!btn) return;
    stopAll(ev);
    var row = btn.closest('.cv-row'); if (!row) return;
    var agent = parseAgentFromRow(row);
    var domain = guessDomainFromRow(row);
